...
export default function Catalog() {
  ...
  const [activeMenu, setActiveMenu] = useState(null); // Hangi menünün açık olduğunu kontrol et
  const [selectedOptions, setSelectedOptions] = useState({}); // Seçili radio butonlarını tutacak state
  const [searchTerms, setSearchTerms] = useState({}); // Arama terimlerini tutacak state

  const toggleDropdown = (menu) => {
    if (activeMenu === menu) {
      setActiveMenu(null); // Eğer tıklanan menü zaten açıksa kapat
    } else {
      setActiveMenu(menu); // Değilse tıklanan menüyü aç
    }
  };

  const handleRadioChange = (menu, value) => {
    setSelectedOptions({ ...selectedOptions, [menu]: value });
  };

  const handleSearchChange = (menu, value) => {
    ...
    <Row className="mt-3">
      ...
      {[
        ...
        <div key={item} className="ct-dropdown">
          <div className="ct-dropdown-header" onClick={() => toggleDropdown(item)}>
            {item}
            <span
                ...
                <>
                  ...
                  <label className="ct-dropdown-item">
                    <input
                      ...
                      onChange={() => handleRadioChange(item, "Eğitmen Dojo")}
                    />
                  ...
                  <label className="ct-dropdown-item">
                    <input
                      ...
                      onChange={() => handleRadioChange(item, "Roiva Eğitmen")}
                    />
                  ...
                  <label className="ct-dropdown-item">
                    <input
                      ...
                      onChange={() => handleRadioChange(item, "Veli Bahçeci")}
                    />
